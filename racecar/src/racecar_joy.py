...
global last_turn_add_once



def callback(data):
    global last_speed_add_once
    twist = Twist()   
    car_twist = Twist() 
    ...
    if (data.buttons[6] == 1 or data.buttons[7] == 1):
        ...
        twist.angular.z = data.axes[3] * turn_add_once
        control_speed = (data.axes[1] * (speed_max - speed_min) * speed_add_once / 2 + speed_mid)  
        if(control_speed > 1500):
            control_speed = control_speed + 350
        elif(control_speed < 1500):
            control_speed = control_speed - 50
        control_turn = (data.axes[3] * (turn_max - turn_min) * turn_add_once  / 2 + turn_mid) 
